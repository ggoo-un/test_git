...
class WDENet(nn.Module):
    def __init__(self, channels):
        ...
        n_head = []
        n_head.append(
            nn.Conv2d(in_channels= 4 * channels, out_channels = features, kernel_size=kernel_size, padding=padding,
                      bias=False))
        ...
        m_tail = [BBlock(features, 4 * channels, 3)]
        ...
        for _ in range(15):
            ...
            layers2.append(nn.ReLU(inplace=True))
        layers2.append(nn.Conv2d(in_channels=features, out_channels=channels, kernel_size=kernel_size, padding=padding,
                                 bias=False))

        op_layer = []
        op_layer.append(nn.Conv2d(in_channels=channels, out_channels=features, kernel_size=kernel_size, padding=padding,
                                 bias=False))
        ...
        self.oplayer = nn.Sequential(*op_layer)
    ...
    def forward(self, x):
        ...
        # plt.show()
        #
        # result_img = utils.make_grid(w_x.data, nrow=8, normalize=True, scale_each=True)
        # result_img = torch.clamp(result_img * 255, 0, 255)
        # result_img = np.uint8(result_img.cpu())
        # a = np.transpose(result_img, (1, 2, 0))
        # cv2.imwrite('test1.jpg', a)

        out0 = self.step_2(w_x)
        out2 = self.step_1(residual)
        oplayer_ = self.oplayer(residual - out2)
        data_cat2 = torch.cat((out0, oplayer_), 1)
